...
/* --------------------------------------------------------------------
   1b. PROJECTS — self-directed projects with a write-up per subtopic
   -------------------------------------------------------------------- */
const projects = [
  {
    title: "SEO — Built This Entire Portfolio Around SEO Standards",
    intro:
      "I had built this entire portfolio you are currently looking at is to get real hands-on experience in SEO instead of just theory. Every part of this portfolio is built from planning to publishing on proper SEO standards.",
    subtopics: [
      {
      ...
      },
      {
        ...
        text: "I found the most relevant keywords with medium and low competition that fit my content, then used them to write my meta titles, meta descriptions, and the page content itself without stuffing keywords unnaturally. I also added proper alt text for every image on this portfolio site."
      },
      {
        heading: "Technical SEO",
        text: "Integrated Google Analytics and Google Search Console to check whether my pages were getting indexed. I set up an XML sitemap and an image sitemap, added a robots.txt file to stop unnecessary pages from being crawled, added FAQ schema so Google can show my FAQs directly in search results, and added a canonical tag to avoid duplicate content issues."
      },
      {
        heading: "Off-Page SEO",
        text: "Finally, I submitted my portfolio link by answering relevant questions on Quora and Reddit to generate backlinks — and reached 138 organic event count from real users."
      }
    ]
  },

  {
    title: "Email Marketing - Zoho Campaigns",
    sectionLabel: "2. Email Marketing - Zoho Campaigns",
    intro:
      "To build hands-on experience in Email Marketing with powerful automation tool knowledge, I explored Zoho Campaigns - from list building to automation.",
    subtopics: [
      { heading: "Contact List Creation", text: "Built a contact list named \"X Company Email Marketing Project\" to organize those contacts for outreach." },
      { heading: "Topic & Campaign", text: "Created a topic called \"Welcome + Offer Emails\" to send emails for the people who are logged-in and set up a campaign named \"Welcome Email.\" Wrote a subject line within 5-7 words, optimized for a high open rate, along with a preheader to boost visibility in the inbox." },
      { heading: "Sender & Receiver Configuration", text: "Explored the sender and receiver settings inside Zoho Campaigns to get familiar with user interface." },
      { heading: "Email Body Design", text: "Designed the email body using Zoho Campaigns' editor - added the brand logo, a personal greeting, a clear explanation of the service offered, a FOMO-based offer to drive urgency and a strong call-to-action to close." },
      { heading: "Automation Workflow", text: "Set up an automation workflow to send a Welcome email with an offer automatically to the person who filled out a sign-up form." },
      { heading: "Acquired knowledge", text: "Learned that Zoho Campaigns requires a business email domain to send emails - a personal Gmail address isn't supported for sending. Since I only had access to Gmail, I explored the entire platform hands-on (list building, campaign creation, content design, automation) without executing the final send step." }
    ]
  }
...
const skillCategories = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Email Marketing", note: "From creating list to automation using Zoho Campaigns, well in writting perfect welcome, sales emails by understanding the services or products with catchy hook, mentioning problem + solution in body with clear call-to-action." },
...
/* --------------------------------------------------------------------
   4. BLOG POSTS — one block per post. Click expands/collapses it.
   -------------------------------------------------------------------- */
const blogPosts = [
  {
    ...
    coverAlt: "SEO",
    sections: [
      { heading: "Sometimes You Need Some Patience", text: "You did proper SEO from top to end for your website, but your site may be new, or you did SEO just a day before. Google actually needs some time to read your site and decide whether to rank it or not. Better to give it some time before any conclusions." },
...

  {
    ...
    conclusionText: "Feel free to reach out to me on LinkedIn. I'll help you turn your clicks into real sales."
  },

  /* Add your next blog post here — copy the block above this line. */
];

/* --------------------------------------------------------------------
   5. FAQ — one block per question
   -------------------------------------------------------------------- */
const faqData = [
  {
  ...
  },
  {
    ...
    answer: "It depends up on the Products or Services. Google Ads works well when people are actively searching for something - that's high intent. Meta Ads is stronger for warm up, engagement, and visual story telling when the audience isn't searching yet but they are our targeted audience."
  },
  {
  ...
  /* Add your next FAQ here — copy the block above this line. */
];
...
projectList.innerHTML = projects.length
  ? projects.map(p => `
    ${p.sectionLabel ? `<h3 class="project-repeat-heading">${p.sectionLabel}</h3>` : ""}
    <article class="project-card">
      <div class="project-subtopics">
        <div class="project-subtopic project-subtopic-intro">
          <div class="project-subtopic-heading">Intro</div>
          <p class="project-subtopic-text">${p.intro}</p>
        </div>
        ${p.subtopics.map(s => `
          <div class="project-subtopic">
            <div class="project-subtopic-heading">${s.heading}</div>
            <p class="project-subtopic-text">${s.text}</p>
          </div>
        `).join("")}
      </div>
    </article>
...
blogList.innerHTML = blogPosts.length
  ? blogPosts.map((b, i) => `
    <article class="blog-card" id="blog-${i}">
      <div class="blog-cover-wrap">
        <span class="blog-tag-float">${b.tag}</span>
        <img src="${b.cover}" alt="${b.coverAlt || b.title}">
      </div>
      <button class="blog-card-trigger" onclick="document.getElementById('blog-${i}').classList.toggle('open')">
        <div class="blog-card-head-row">
          <div class="blog-card-head">
            <h3>${b.title}</h3>
            <p class="blog-hook">${b.hook}</p>
          </div>
          <span class="blog-read-more">Read post <span class="blog-chevron">&#9662;</span></span>
        </div>
      </button>
      <div class="blog-body">
        <div class="blog-body-inner">
          ${b.sections.map(s => `
            <div class="blog-section">
              <div class="blog-section-heading">${s.heading}</div>
              <p class="blog-section-text">${s.text}</p>
            </div>
          `).join("")}
  ...
  `).join("")
  : `<p class="published-empty">Blog posts will appear here.</p>`;

  // --- FAQ ---
const faqList = document.getElementById("faq-list");
faqList.innerHTML = faqData.length
  ? faqData.map((f, i) => `
    <article class="faq-card" id="faq-${i}">
      <button class="faq-card-trigger" onclick="document.getElementById('faq-${i}').classList.toggle('open')">
  ...
  `).join("")
  : `<p class="published-empty">FAQs will appear here.</p>`;
